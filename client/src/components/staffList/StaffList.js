...
import {deleteStaffById, getListStaff, toggleModal, toggleView} from "../../features/StaffInfoSlice"
import {toast, ToastContainer} from "react-toastify"
import {useNavigate} from "react-router-dom"
import StaffFormSearch from "../staffForm/StaffFormSearch"
import StaffFormView from "../staffForm/StaffFormView"
const StaffList = () => {
  const {staff, view , modal, loading} = useSelector(state => state.staff)
  ...
  return (
    <>
      ...
      {
        !loading ? 
         <div className="flex flex-wrap gap-2 justify-center items-center py-72">
          <Spinner color="info"/>
         </div> 
         : 
         ...
         {staff?.length > 0 && staff?.map((i, index) => {
          return(
           <div key={index}>
             ...
             <Modal show={modal} popup={true} onClose={handleConfirmStaffDelete}>
               <Modal.Header/>
               <Modal.Body>
             ...
             <Modal show={view} popup={true} onClose={handleView}>
               <Modal.Header/>
               <Modal.Body>
                 {edit && <StaffFormView data={edit}/>}
               </Modal.Body>
             </Modal>
           </div>
          )
